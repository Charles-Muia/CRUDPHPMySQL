...
function insertData($conn, $u_name, $u_email, $u_card, $u_mobile)
{
    ...
    elseif (!filter_var($u_email, FILTER_VALIDATE_EMAIL)) {
        ...
        $query = mysqli_query($conn, "INSERT INTO `users`(`name`,`email`,`number`,`mobile`) VALUES('$u_name','$u_email','$u_card','$u_mobile')");
        // IF USER INSERTED
        if ($query) {
            return true;
        }
