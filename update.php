...
function updateUser($conn, $id, $u_name, $u_email, $u_card, $u_mobile)
{
    ...
    $u_card = trim(mysqli_real_escape_string($conn, htmlspecialchars($u_card)));
    $u_mobile = trim(mysqli_real_escape_string($conn, htmlspecialchars($u_mobile)));

    // IF NAME OR EMAIL IS EMPTY
    if (empty($u_name) || empty($u_email) || empty($u_card) || empty($u_mobile)) {
    ...
    elseif (!filter_var($u_email, FILTER_VALIDATE_EMAIL)) {
        ...
        // UPDATE USER DATA
        $query = mysqli_query($conn, "UPDATE `users` SET `name`='$u_name', `email`='$u_email', `number`='$u_card', `mobile`='$u_mobile' WHERE `id`='$id'");
